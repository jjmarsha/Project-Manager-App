		...
		try {
			...
			if (!rs.next()) {
				...
				PreparedStatement getUserID = connection
						.prepareStatement("SELECT u.userID FROM user u WHERE u.userName = ?");
				...
				PreparedStatement ps2 = connection.prepareStatement(
						"SELECT uip.userName FROM usersInProject uip, project p WHERE uip.userName = ? AND uip.projectID = p.projectID AND p.projectID = ?");
				ps2.setString(1, user);
				ps2.setInt(2, projectID);
				ResultSet rs2 = ps2.executeQuery();
				if (!rs2.next()) {
					error err = new error("User does not belong to project!");
					String json = new Gson().toJson(err);
					response.getWriter().write(json);
				} else {
					error err = new error("User is validated!");
					String json = new Gson().toJson(err);
					response.getWriter().write(json);
				}
			}
